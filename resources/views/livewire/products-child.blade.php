...
<flux:modal name='products-child' class="relative !max-w-none" wire:close="$dispatchTo('products-child', 'resetForm');$refresh">
    <div class="bg-white rounded-lg shadow-sm max-w-5xl mx-auto pt-4 min-w-[750px] min-h-[400px] flex flex-col">
        {{-- Loading Indicator --}}
        <div x-show="$wire.state === null" class="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2">
        ...
        <div x-show="$wire.state !== null" class="flex flex-col flex-1 -mt-4">

            {{-- Show --}}
            @if ($state === 'Show' && $product !== null)
                <form method="POST" action="{{ route('orders.store') }}" class="flex-1 flex flex-col lg:flex-row gap-8 max-w-[750px]">
                    @csrf
            ...
            @elseif (in_array($state, ['Create', 'Edit']))
                ...
                <div class='max-w-[800px]'>
                    ...
                    <div class="mb-5 w-full">
                        ...
                        <flux:dropdown>
                            <flux:button id='category' variant='subtle' icon:trailing="chevron-down" class="pl-2 w-full border border-zinc-200 !justify-start">
